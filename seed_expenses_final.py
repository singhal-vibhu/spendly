...
def seed_expenses(user_id, count, months):
    ...
    try:
        ...
        for row in sample_query:
            # Using 'INR' instead of the rupee symbol to avoid encoding issues on Windows
            print(f"ID: {row['id']} | {row['date']} | {row['category']}: INR {row['amount']} - {row['description']}")
